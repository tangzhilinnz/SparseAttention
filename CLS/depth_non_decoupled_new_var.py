...
width = 256# 128,256
depth = 9 #3,6,9
# base_lrs = [0.03162278, 0.04582591, 0.06640828, 0.09623506, 0.13945832,
#        0.20209499, 0.29286446, 0.42440235, 0.6150195 , 0.89125094]#[0.1,0.25,0.5,0.75,1.0,1.25,1.5,1.75]\
set_variance = 1.0
base_lrs = [0.2,0.4,0.6,0.8]
epochs = 30
seed = 1
T = 1
seed = 1
...
class DecoupledFNNBlockFn(Function):
    ...
    def forward(ctx, h, U_fwd, U_bwd, W_fwd, W_bwd, U_fwd_copy, W_fwd_copy,
        ...
        # We save 'h' and 'W' for the backward pass. 
        # U is no longer used, but we keep the argument signature consistent.
        #ctx.save_for_backward(h, U_fwd, U_bwd, W_fwd, W_bwd, U_fwd_copy, W_fwd_copy)
        ctx.save_for_backward(h, W_fwd)
        ctx.alpha = alpha
        ...
        # 1. Apply activation directly to input h (Skipping U projection)
        a = phi(h)
    ...
    def backward(ctx, grad_y, grad_x_fwd_diff=None, grad_y_fwd_diff=None):
        h, W_fwd = ctx.saved_tensors
        alpha = ctx.alpha
        phi = ctx.phi
        phi_prime = ctx.phi_prime

        # --- UPDATED BACKWARD FORMULA ---
        
        # 1. Recompute activation 'a' and its derivative w.r.t 'h'
        a = phi(h)
        da_dh = phi_prime(h) # derivative of phi(h) w.r.t h

        # 2. Gradient for W_fwd
        # Standard: alpha * (grad_y^T @ a)
        grad_W_fwd = alpha * torch.einsum("...i,...j->ij", grad_y, a)

        # 3. Gradient w.r.t 'h' (Input)
        # Chain rule: 
        # Part A (Residual): grad_y
        # Part B (Branch):   grad_y @ W_fwd * phi'(h)
        
        grad_a = alpha * (grad_y @ W_fwd) # Backprop through W
        grad_h_from_branch = grad_a * da_dh # Backprop through Activation
        
        grad_h = grad_y + grad_h_from_branch
...
class DecoupledAttBlockFn(Function):
    ...
    def forward(
        ...
        # Save tensors (Removed v_fwd, v_bwd, v_fwd_copy as they are unused)
        ctx.save_for_backward(h, q_fwd, k_fwd, o_fwd, q_bwd, k_bwd, o_bwd, q_fwd_copy, k_fwd_copy, o_fwd_copy)
        ctx.alpha = alpha
        ctx.inv_sqrt_n = inv_sqrt_n
    ...
    def backward(ctx, grad_y, q_diff=None, k_diff=None, v_diff=None, logits_diff=None, attn_diff=None):
        # Retrieve saved tensors (v_* are gone)
        h, q_fwd, k_fwd, o_fwd, q_bwd, k_bwd, o_bwd, q_fwd_copy, k_fwd_copy, o_fwd_copy = ctx.saved_tensors
        alpha = ctx.alpha
        ...
        grad_V = torch.einsum("bhqk, bhqd -> bhkd", A, grad_out) # Gradients for V values
        ...
        return (
            grad_h,
            grad_q_fwd, grad_k_fwd, None, grad_o_fwd, # v_fwd is None
            None, None, None, None,                   # bwd grads
            None, None, None, None,                   # copies
            grad_alpha,
            grad_inv_sqrt_n,
            grad_num_heads,
            grad_softmax,
...
class DecoupledAttention(nn.Module):
    ...
    def forward(self, h):
        # We pass None for all V-related arguments
...
for base_lr in base_lrs:
    ...
    #optimizer = optim.SGD(model.parameters(), lr=base_lr*width)
