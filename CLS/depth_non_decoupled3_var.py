...
def set_optimizer_with_different_lr_decoupled(model, width, depth, base_lr=0.0):
    """
    Applies lr = base_lr * width to all standard/forward parameters.
    Excludes all backward (_bwd) weights from the optimizer so they do not update.
    """
    uniform_lr = base_lr * width
    params_to_update = []
    
    for name, param in model.named_parameters():
        # If "_bwd" is in the parameter name, skip it entirely
        if "_bwd" in name:
            continue
        
        # Otherwise, add it to the list of parameters to update
        params_to_update.append(param)
            
    # Initialize SGD with only the filtered parameters and the uniform learning rate
    optimizer = optim.SGD(params_to_update, lr=uniform_lr)
